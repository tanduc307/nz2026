...
# write.xlsx(df_reg, "df_reg.xlsx")

library(readxl)

df_reg <- read_excel("df_reg_chuan.xlsx")
```
...
unique(df_long$SCFA) -> vec_SCFA

df_term_ls <- list()

for(i in 1:length(vec_SCFA)){

df_out_1 <- df_long |> subset(SCFA == vec_SCFA[i] & Termness == "term")

df_out_1 <- df_out_1 |> subset(value != 0)

a <- boxplot(df_out_1$value)

df_out_1 <- df_out_1[ which(!(df_out_1$value %in% a$out)) , ]

df_term_ls[[i]] <- df_out_1
}

df_term_ls_final <- do.call(rbind, df_term_ls)
```
...
df_out_1 <- df_long |> subset(SCFA == vec_SCFA[i] & Termness == "preterm")
...
df_preterm_ls[[i]] <- df_out_1

print(a$out)
}

df_preterm_ls_final <- do.call(rbind, df_preterm_ls)
```


```{r}
df_long <- rbind(df_term_ls_final, df_preterm_ls_final)
dim(df_long)
dim(df_long_ori)
```
...
```{r}
fit_acetate <- lm(acetate_meconium ~ Gestation,
                  data = df_reg_meconium)

summary(fit_acetate)

fit_propionate <- lm(propionate_meconium ~ Gestation,
                  data = df_reg_meconium)

summary(fit_propionate)

fit_isobutyrate <- lm(isobutyrate_meconium ~ Gestation,
                  data = df_reg_meconium)

summary(fit_isobutyrate)

fit_butyrate <- lm(butyrate_meconium ~ Gestation,
                  data = df_reg_meconium)
...
```{r}
fit_acetate <- lm(acetate_stool ~ Gestation,
                  data = df_reg_stool)

summary(fit_acetate)

fit_propionate <- lm(propionate_stool ~ Gestation,
                  data = df_reg_stool)

summary(fit_propionate)

fit_isobutyrate <- lm(isobutyrate_stool ~ Gestation,
                  data = df_reg_stool)

summary(fit_isobutyrate)

fit_butyrate <- lm(butyrate_stool ~ Gestation,
                  data = df_reg_stool)
